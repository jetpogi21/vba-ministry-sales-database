...
Public Function SelectAllContent(frm, controlName)
    Dim txtBox As TextBox: Set txtBox = frm(controlName)
    txtBox.SetFocus
    txtBox.SelStart = 0
...
Public Function GetKVPairs(QueryName, rs As Recordset) As String
    ...
    Dim rs2 As Recordset: Set rs2 = ReturnRecordset("SELECT * FROM qryConfigEnumeratorFields WHERE Not Exclude AND QueryName = " & Esc(QueryName) & " ORDER BY ConfigEnumeratorFieldID")
...
Public Sub onerror()
    ...
    arr.Add "On Error GoTo ErrHandler:"
    arr.Add vbTab & "Exit Function"
    arr.Add vbTab & "ErrHandler:"
    arr.Add vbTab & "If Err.Number = 2450 Then"
    arr.Add vbTab & "Exit Function"
...
Public Sub a(str, Optional FunctionName = "")
    ...
    Dim arr As New clsArray
    arr.Add "Dim " & str & " as new clsArray"
...
Public Sub dv(str)
    ...
    Dim arr As New clsArray
    arr.Add "Dim " & str & ": " & str & " = rs.Fields(" & Esc(str) & ")"
    arr.Add "If ExitIfTrue(IsFalse(" & str & ")," & Esc(str & " is empty..") & ") Then exit function"
