...
Public Function CreateFlexControl(objectName, ControlType, Optional Section As AcSection = acDetail, Optional parent, _
    ...
    Dim ctl As control
    If isForm Then
        Set ctl = CreateControl(objectName, ControlType, Section, , ColumnName, Left, Top, Width, Height)
    Else
        Set ctl = CreateReportControl(objectName, ControlType, Section, , ColumnName, Left, Top, Width, Height)
    End If
...
Public Sub CreateContinuousFormDeleteButton(frm As Form, ModelID)
...
Public Sub CreateContinuousFormButton(frm As Form, standardHeight, ControlSource, TextBoxName, ButtonName)
